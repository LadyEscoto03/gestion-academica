<div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
    ...
    <div>
        <div class="py-3">
            <a href="{{route('disabilities.create')}}"
                ...
                role="button">Crear discapacidad</a>
        </div>
        <x-table.table :headers="['Id', 'Nombre', 'Tipo', 'Acciones']">
            @foreach ($disabilities as $disability)
                <x-table.tr>
                    <x-table.td> {{ $disability->id }} </x-table.td>

                    <x-table.td> {{ $disability->name }}</x-table.td>
                    <x-table.td> {{ $disability->type }} </x-table.td>
